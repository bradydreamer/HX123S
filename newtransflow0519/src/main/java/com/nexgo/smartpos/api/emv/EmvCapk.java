...
public class EmvCapk implements Parcelable {
	...
	/** 认证中心公钥指数 */
	private byte[] CAPKExponent = new byte[4];
...
//	};
	...
	public void writeToParcel(Parcel dest, int flags) {
		...
		dest.writeByte(this.CA_PKIndex);
		dest.writeByte(this.CA_HashAlgoIndicator);
		dest.writeByte(this.CA_PKAlgoIndicator);
		dest.writeInt(this.LengthOfCAPKModulus);
		dest.writeByteArray(this.CAPKModulus);
		dest.writeInt(this.LengthOfCAPKExponent);
		dest.writeByteArray(this.CAPKExponent);
		dest.writeByteArray(this.ChecksumHash);
		dest.writeByteArray(this.CAPKExpDate);
	}

	public EmvCapk() {
	}

	protected EmvCapk(Parcel in) {
		this.RID = in.createByteArray();
		this.CA_PKIndex = in.readByte();
		this.CA_HashAlgoIndicator = in.readByte();
		this.CA_PKAlgoIndicator = in.readByte();
		this.LengthOfCAPKModulus = in.readInt();
		this.CAPKModulus = in.createByteArray();
		this.LengthOfCAPKExponent = in.readInt();
		this.CAPKExponent = in.createByteArray();
		this.ChecksumHash = in.createByteArray();
		this.CAPKExpDate = in.createByteArray();
	}

	public static final Creator<EmvCapk> CREATOR = new Creator<EmvCapk>() {
		public EmvCapk createFromParcel(Parcel source) {
			return new EmvCapk(source);
		}
