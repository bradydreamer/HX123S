...
jint Java_com_nexgo_jniinterface_CallNative_UnPackage(JNIEnv*  env,jobject this , jcharArray jpackage, jint jplen,jcharArray jtlvdata, jcharArray datalen)
{
	...
	Log("UnPackage","tlvlen=%d",tlvlen[0]);
	Log("UnPackage","tlvdata=%s",tlvdata);
	if(tlvlen[0]!=0)
	{
		(*env)->SetByteArrayRegion(env, jtlvdata, 0, tlvlen[0], (jbyte *) tlvdata);
		(*env)->SetByteArrayRegion(env, datalen, 0, 1, (jbyte *) tlvlen);
	}
...
jint Java_com_nexgo_jniinterface_CallNative_jnitest(JNIEnv*  env,jobject this )
{
    ...
    ProviderSetRecordByVoucherNo("000001");
	return 88;
}
