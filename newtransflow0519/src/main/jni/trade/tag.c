...
void FreeTagList(TagList *pTagList)
{
	...
	for (indexr=0; indexr<pTagList->ItemCount; indexr++)

	{

		sdkFreeMem(pTagList->tagItem[indexr].Value);
...

{
	...
	for (indexr=0; indexr<sourceTagList->ItemCount; indexr++)

	{

		SetTagValue(sourceTagList->tagItem[indexr].Tag,

			sourceTagList->tagItem[indexr].Value,

			sourceTagList->tagItem[indexr].Length, descTagList);
...

{
	...
	for (indexr=0; indexr<pTagList->duplicateItemCount; indexr++)

	{

		if (tag==pTagList->duplicateItem[indexr])
...
*/
    ...
    {
        sdkFreeMem(pTagList->tagItem[GetTagIndex(pTagList, tag)].Value);
        pTagList->tagItem[GetTagIndex(pTagList, tag)].Value = NULL;
...
u32 BuildTagListInMode(u8 *buf, u32 size, u32 mode, TagList *pTagList)
{
	...
	while(CurPos < EndPos)
	{
		...
		if(SDK_OK != ParseTlvInfo(CurPos, (u16)(EndPos-CurPos), &Tag, &TagSize, &Length, &lengthSize))
...

{
	...
	for	(indexr = 0; indexr < tagCount; indexr++)

	{

		if (tag == tagTypeArray[indexr].Tag)

		{

			return tagTypeArray[indexr].Type;
...

{
...

	{
		...
		for (indexr=tagIndex; indexr<pTagList->ItemCount-1; indexr++)

		{

			memcpy(&pTagList->tagItem[indexr], &pTagList->tagItem[indexr+1], sizeof(TagItem));
