...
public class DataExchangeService extends Service {
	public static String ip = "223.72.175.128"; //"10.2.1.25";
	public static int port = 19004;  //8000

//	public static String ip = "10.2.1.25";
...
//		}
		...
		public byte[] exchangeData(byte[] data) throws RemoteException {
			...
			try {
				...
				Log.e("socket_connect_host", "开始连接" + ip);
				socket_connect_host = new Socket(ip, port);
				...
				outputStream.flush();
				Log.e("data", "socket_connect_host往收单平台发送数据：[ " + data.length+" ] DATA = "+Arrays.toString(data));
			...
			try {
				...
				Log.e("data", "SDK 从收单平台返回的数据：[ " + dataSendToServer.length + "] DATA = "+ Arrays.toString(dataSendToServer));
