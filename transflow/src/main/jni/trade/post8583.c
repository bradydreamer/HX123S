...
s32 InitFormSt8583(SDK_8583_ST8583 *pstBag8583, struct _TradeData *pstTradeData) {
	...
	/*
	 ...
	 {
	 pstBag8583->ucBagData[pstBag8583->nBagLen++] = 0x61;                                             //IC锟斤拷应锟斤拷
	 }
	 else
	 {
	 pstBag8583->ucBagData[pstBag8583->nBagLen++] = 0x60;                                             //锟脚匡拷应锟斤拷
	 }
	 */
...
s32 Set8583Tvl(SDK_8583_ST8583 *pstBag8583) {
	...
	for (i = 2; i <= uiMaxFieldNo; i++) {
		...
		case 11:
			if (sdk8583IsDomainExist(pstBag8583, i)) {
				slt = GetFieldData(pstBag8583, i, proccode, 256);
				ProviderSetTagValue(TAG_8583_VoucherNo, proccode, slt);
			}
			break;
		case 12:
			if (sdk8583IsDomainExist(pstBag8583, i)) {
	...
	if (!IsMobileTypeTrade(stTradeData->ucTradeId)) {
		if (sdk8583IsDomainExist(pstRevBag8583, 55))          //鍒ゆ柇55鍩熸槸鍚︽暟鎹?
			...
			if (len > 0) {
				...
				eEmvRet = ProviderSecAuth(asRespCode, asAuthCode, revtmp, len,
						pOutData, &pOutDatalen);      //Fixme 二次授权  返回数据
				if (eEmvRet < 0) {
					return SDK_ERR;
				} else {
				...
				Log("wangb", "ProviderSecAuth is %d ", eEmvRet);
			} else {
				return SDK_ERR;
			}
		}
 ...
 ** Remarks: 		参数会进行修改
 *****************************************************************************/
s32 PostPag55Data(SDK_8583_ST8583 * pstRevBag8583,
		struct _TradeData *stTradeData) {
	s32 nRet = 0;
	s32 eEmvRet = 0;
	u8 pOutData[512] = { 0 };
...
/*****************************************************************************
 锟斤拷 锟斤拷 锟斤拷  : DealPortionTrade
 锟斤拷锟斤拷锟斤拷锟斤拷  : 锟斤拷锟斤拷锟街承兑斤拷锟斤拷
 锟斤拷锟斤拷锟斤拷锟? : 锟斤拷
 锟斤拷锟斤拷锟斤拷锟? : 锟斤拷
 锟斤拷 锟斤拷 值  :
 锟斤拷锟矫猴拷锟斤拷  :
...
/*****************************************************************************
 ** Descriptions:	锟斤拷锟斤拷锟接★拷锟街?
 ** Parameters:    	void
 ** Returned value:
 ** Created By:		lilin  2015.05.18
 ** Remarks:
 *****************************************************************************/
/*****************************************************************************
 锟斤拷 锟斤拷 锟斤拷  : DealTradeReponse
 锟斤拷锟斤拷锟斤拷锟斤拷  : 锟斤拷锟斤拷锟斤拷应锟斤拷锟斤拷
 锟斤拷锟斤拷锟斤拷锟? : const s32 siRet
 锟斤拷锟斤拷锟斤拷锟? : 锟斤拷
 锟斤拷 锟斤拷 值  :
 锟斤拷锟矫猴拷锟斤拷  :
 锟斤拷锟斤拷锟斤拷锟斤拷  :
...
//锟斤拷锟叫的达拷锟斤拷锟斤拷//zxx 20130802 15:3
//锟斤拷siRet = sdk_equ时锟斤拷锟斤拷锟斤拷锟斤拷锟绞猴拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟?
/*****************************************************************************
 ** Descriptions:	锟斤拷锟斤拷锟阶达拷锟斤拷锟斤拷应
 ** Parameters:    	const s32 siRet
 ** Returned value:
 ** Created By:		lilin2012.12.09
 ** Remarks:
 *****************************************************************************/
...
/*****************************************************************************
 ** Descriptions:
 ** Parameters:    	const s32 ucIndex
 SDK_ICC_CARDDATA *pstCardData
 ** Returned value:
 ** Created By:		lilin  2015.06.06
 ** Remarks:
 *****************************************************************************/
